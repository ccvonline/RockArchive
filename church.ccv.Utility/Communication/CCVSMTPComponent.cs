...
namespace church.ccv.Utility.Communication
{
    ...
    /// </summary>
    public abstract class CCVSMTPComponent : Rock.Communication.TransportComponent
    {
        #region Fields
        ...
        /// <returns></returns>
        public bool ValidRecipient( CommunicationRecipient recipient, bool isBulkCommunication, Category category )
        {
            bool valid = ValidRecipient( recipient, isBulkCommunication );

            // if valid, a bulk communication, and category specified 
            // ensure the person hasnt opted out of the marketing category
            if ( valid && isBulkCommunication && category != null )
            {
                ...
                if ( person != null )
                {
                    ...
                    {
                        recipient.Status = CommunicationRecipientStatus.Failed;
                        recipient.StatusNote = String.Format( "Marketing Preference of 'No {0} Bulk Communications'", category.ToString() );
                        valid = false;
                    }
                }                
            }
        ...
        public override bool ValidRecipient( CommunicationRecipient recipient, bool isBulkCommunication )
        {
            ...
            if ( valid )
            {
                ...
                if ( person != null )
                {
                    ...
                    else if ( person.EmailPreference == Rock.Model.EmailPreference.DoNotEmail )
                    {
                        recipient.Status = CommunicationRecipientStatus.Failed;
                        recipient.StatusNote = "Communication Preference of 'Do Not Send Communication'";
                        valid = false;
                    }
                    else if ( person.EmailPreference == Rock.Model.EmailPreference.NoMassEmails && isBulkCommunication )
                    {
                        recipient.Status = CommunicationRecipientStatus.Failed;
        ...
        private void CheckSafeSender( MailMessage message, GlobalAttributesCache globalAttributes )
        {
            if ( message != null && message.From != null )
            {
                ...
                var safeDomainValues = DefinedTypeCache.Read( Rock.SystemGuid.DefinedType.COMMUNICATION_SAFE_SENDER_DOMAINS.AsGuid() ).DefinedValues;
