...
// CCV Fork - Added option to include child locations when adding locations from modal to checkin area - cfu 5-24-19
//          - Added link button to remove all locations from checkin area - cfu 5-24-19

namespace RockWeb.Plugins.church_ccv.CheckIn.Config
{
    [DisplayName( "CCV Check-in Areas" )]
    [Category( "CCV > Check-in > Configuration" )]
    [Description( "Configure Check-in areas and groups." )]
    public partial class CCVCheckinAreas : RockBlock, ISecondaryBlock
    {
        ...
        protected void mdLocationPicker_SaveClick( object sender, EventArgs e )
        {
            ...
            Location selectedLocation = new LocationService( new RockContext() ).Get( locationPicker.SelectedValue.AsInteger() );

            if ( selectedLocation != null )
            {
                locations.Add( selectedLocation );

                if ( rcbIncludeChildLocations.Checked == true && selectedLocation.ChildLocations.Count > 0 )
                {
                    // include all child locations
                    List<Location> childLocations = GetChildLocations( selectedLocation );

                    locations.AddRange( childLocations );
                }
        ...
        /// <summary>
        /// Handles the remove all locations click event
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void btnRemoveAllLocations_Click( object sender, EventArgs e )
        {
            checkinGroup.Locations.Clear();
        ...
        private void SelectArea( Guid? groupTypeGuid )
        {
            ...
            btnRemoveAllLocations.Visible = false;
            ...
            if ( groupTypeGuid.HasValue )
            {
                using ( var rockContext = new RockContext() )
                {
                    ...
                    if ( groupType != null )
                    {
                        ...
                        btnRemoveAllLocations.Visible = true;
                    }
                    else
                    {
...
", hfIsDirty.ClientID, lbAddArea.ClientID );
        ...
        /// <summary>
        /// Return a list of all child locations (recursive)
        /// </summary>
        /// <param name="topLocation"></param>
        /// <returns></returns>
        private List<Location> GetChildLocations( Location topLocation )
        {
            List<Location> locations = new List<Location>();

            foreach ( Location childLocation in topLocation.ChildLocations )
            {
                locations.Add( childLocation );

                if ( childLocation.ChildLocations.Count > 0 )
                {
                    List<Location> childLocations = GetChildLocations( childLocation );
