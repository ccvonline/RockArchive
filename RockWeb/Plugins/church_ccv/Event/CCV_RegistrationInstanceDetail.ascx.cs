...
namespace RockWeb.Plugins.church_ccv.Event
{
    ...
    /// </summary>
    [DisplayName( "CCV Registration Instance Detail" )]
    [Category( "Event" )]
    ...
    public partial class CCV_RegistrationInstanceDetail : Rock.Web.UI.RockBlock, IDetailBlock
    {
        #region Fields
        ...
        private void BindGroupPlacementGrid( bool isExporting = false )
        {
            ...
            if ( instanceId.HasValue )
            {
                using ( var rockContext = new RockContext() )
                {
                    ...
                    // Checks to see if a page parameter "Skip" has a value. If it does, only display 10 registrants at a time.
                    // This gets set by the textbox and apply button on the front end. 
                    int? skip = PageParameter( "Skip" ).AsIntegerOrNull();
                    if ( skip.HasValue )
                    {
                        qry = qry.OrderBy( r => r.PersonAlias.Person.LastName )
                                    .ThenBy( r => r.PersonAlias.Person.NickName )
                                    .Skip( skip.Value ).Take( 10 );
                    }
