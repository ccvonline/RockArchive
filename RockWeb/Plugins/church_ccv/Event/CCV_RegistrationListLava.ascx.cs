...
namespace RockWeb.Plugins.church_ccv.Event
{
    ...
    /// 
    /// </summary>
    [DisplayName("CCV Registration List Lava")]
    [Category("CCV > Event")]
    ...
    public partial class CCV_RegistrationListLava : Rock.Web.UI.RockBlock
    {
        ...
        protected void LoadContent()
        {
            ...
            {
                registrationsList = registrationsList.Where(a => a.BalanceDue != 0).ToList();
            }

            int? maxResults = GetAttributeValue("MaxResults").AsIntegerOrNull();
            if (maxResults.HasValue && maxResults > 0)
            {
                registrationsList = registrationsList.TakeLast(maxResults.Value).ToList();   
            }

            var mergeFields = Rock.Lava.LavaHelper.GetCommonMergeFields(this.RockPage, this.CurrentPerson);
            mergeFields.Add("Registrations", registrationsList);
