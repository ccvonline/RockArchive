...
{
    [DisplayName( "CCV Communication List" )]
    [Category( "CCV >Communication" )]
    [Description( "Lists the status of all previously created communications. CCV Custom: Customized Date range to always reset after a short period of time." )]
    ...
    public partial class CCVCommunicationList : Rock.Web.UI.RockBlock, ICustomGridColumns
    {
        ...
        protected void rFilter_ApplyFilterClick( object sender, EventArgs e )
        {
            ...
            {
                // If a date range has not been selected, default to today
                drpDates.LowerValue = RockDateTime.Today;
            }
        ...
        private void SetFilter()
        {
            ...
            // Custom CCV: 3/6/2020
            // As of this date, CCV creates on average over 1000 communications a day which is putting a lot of
            // stress on this block.  To fix it, we will always reset the date filter to today's date unless
            // the user happens to be filtering by date at the current moment.
            int? personEntityTypeId = Rock.Web.Cache.EntityTypeCache.Read( Person.USER_VALUE_ENTITY ).Id;
            using ( var rockContext = new Rock.Data.RockContext() )
            {
                string key = string.Format( "grid-filter-{0}-Date Range|Date Range", this.BlockId );

                var datePreference = new AttributeValueService( rockContext ).Queryable()
                    .Where( v =>
                        ...
                        v.Attribute.Key == key &&
                        v.EntityId == CurrentPerson.Id )
                    .FirstOrDefault();

                // if the person set the date range filter within the last 30 minutes than use that value
                // if not reset the filter to today
                if (datePreference != null &&
                        (datePreference.CreatedDateTime >= RockDateTime.Now.AddMinutes( -30 ) ||
                         datePreference.ModifiedDateTime >= RockDateTime.Now.AddMinutes( -30 ) )
                    )
                {
                    drpDates.DelimitedValues = datePreference.Value;
                }
                else
                {
                    drpDates.LowerValue = RockDateTime.Today;
