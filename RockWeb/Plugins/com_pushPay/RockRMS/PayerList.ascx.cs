    ...
    public partial class PayerList : Rock.Web.UI.RockBlock
    {
        ...
        private void BindGrid()
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                {
                    payers =  qry.Sort( sortProperty ).ToList();
                }
                else
                {
                    payers = qry.OrderBy( d => d.PayerKey ).ToList();
                }

                rGrid.DataSource = payers.Select( p => new
                {
                    p.Id,
                    p.PayerKey,
                    PersonId = p.PersonAlias.PersonId,
                    Name = p.PersonAlias.Person.FullNameReversed
                }).ToList();
                rGrid.DataBind();
            }
        }
