...
class TwilioResponseAsync : IAsyncResult
{
    ...
    private void MessageRecieved()
    {
        ...
        if( string.IsNullOrEmpty(  request.Form["To"] ) || string.IsNullOrEmpty( request.Form["From"] ) ){
            return;
        }
        ...
        // Check to see if message contains any images.   
        body += GetMMSData();
    ...
    private string GetMMSData()
    {
        string mmsData = string.Empty;
        var request = _context.Request;
        var numMedia = int.Parse( request["NumMedia"] );
        ...
        mmsData = Environment.NewLine + "Attached Images:" + Environment.NewLine;
        for ( int i = 0; i < numMedia; i++ )
        {
            if ( string.IsNullOrEmpty( request["MediaUrl" + i] ) )
            {
                continue;
            }

            mmsData += request["MediaUrl" + i] + Environment.NewLine;
        }
